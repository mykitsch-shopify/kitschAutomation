...
| major | 0 |
| minor | 2 |
| harness (our own gaps, not store defects) | 13 |
...
- **(run)** (add_to_cart) — add_to_cart failed on 10 of 10 target(s) — every one it looked at, with the same result each time.

Read the 10 finding(s) above as ONE question rather than 10 answers. A check that fails everywhere has two explanations and this run cannot tell them apart:
  ...
  the harness the selector behind "add_to_cart" no longer matches this theme, in which case nothing has been checked and nothing is known.
...
- **(run)** (variations) — variations failed on 10 of 10 target(s) — every one it looked at, with the same result each time.

Read the 10 finding(s) above as ONE question rather than 10 answers. A check that fails everywhere has two explanations and this run cannot tell them apart:

  the store   something used on every one of these pages is genuinely broken, in which case this is the most urgent thing in the report;
  the harness the selector behind "variations" no longer matches this theme, in which case nothing has been checked and nothing is known.
